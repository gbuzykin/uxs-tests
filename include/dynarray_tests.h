...
#define CHECK(...) \
    if (!vector_check(__VA_ARGS__)) { \
        throw std::runtime_error(uxs_test_suite::report_error(__FILE__, __LINE__, "dynarray mismatched")); \
    }
...
#define CHECK_EMPTY(x) \
    { \
        ...
        if (__v.size() != 0 || __v.begin() != __v.end()) { \
            throw std::runtime_error(uxs_test_suite::report_error(__FILE__, __LINE__, "dynarray is not empty")); \
        } \
    }
...
bool vector_check(const uxs::dynarray<Ty, Alloc>& v, size_t sz, InputIt src) {

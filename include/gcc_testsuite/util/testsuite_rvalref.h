...
inline bool operator==(const rvalstruct& lhs, const rvalstruct& rhs) { return lhs.val == rhs.val; }

inline bool operator<(const rvalstruct& lhs, const rvalstruct& rhs) { return lhs.val < rhs.val; }

inline void swap(rvalstruct& lhs, rvalstruct& rhs) {
...
struct copycounter {
    ...
    copycounter(copycounter&& in) noexcept {
        assert(in.valid == true);
        val = in.val;
        in.valid = false;
        valid = true;
    }

    copycounter& operator=(int newval) {
        val = newval;
        valid = true;
        return *this;
    }

    bool operator=(const copycounter& in) {
        VERIFY(in.valid == true);
        ++copycount;
        val = in.val;
        valid = true;
    ...
    copycounter& operator=(copycounter&& in) {
        VERIFY(in.valid == true);
        val = in.val;
        in.valid = false;
        valid = true;
    ...
    ~copycounter() noexcept { valid = false; }
};

inline bool operator==(const copycounter& lhs, const copycounter& rhs) { return lhs.val == rhs.val; }

inline bool operator<(const copycounter& lhs, const copycounter& rhs) { return lhs.val < rhs.val; }

inline void swap(copycounter& lhs, copycounter& rhs) {
    VERIFY(lhs.valid && rhs.valid);
    int temp = lhs.val;
    lhs.val = rhs.val;
    rhs.val = temp;
...
struct rvalstruct_compare_by_value {
    ...
    rvalstruct_compare_by_value(int v) : val(v), ok(true) {}

    rvalstruct_compare_by_value(const rvalstruct_compare_by_value& rh) : val(rh.val), ok(rh.ok) { VERIFY(rh.ok); }

    rvalstruct_compare_by_value& operator=(const rvalstruct_compare_by_value& rh) {
        VERIFY(rh.ok);
        val = rh.val;
        ok = rh.ok;
        return *this;
    ...
    rvalstruct_compare_by_value(rvalstruct_compare_by_value&& rh) : val(rh.val), ok(rh.ok) {
        VERIFY(rh.ok);
        rh.ok = false;
    }

    rvalstruct_compare_by_value& operator=(rvalstruct_compare_by_value&& rh) {
        VERIFY(rh.ok);
        val = rh.val;
        ok = rh.ok;
        rh.ok = false;
...
inline bool operator<(rvalstruct_compare_by_value lh, rvalstruct_compare_by_value rh) {
    VERIFY(rh.ok);
    VERIFY(lh.ok);
    return lh.val < rh.val;
}

inline bool order(rvalstruct_compare_by_value lh, rvalstruct_compare_by_value rh) {
    VERIFY(rh.ok);
    VERIFY(lh.ok);
    return lh.val < rh.val;
}
...
struct throwing_move_constructor {
    ...
    throwing_move_constructor(throwing_move_constructor&&) { throw 1; }

    throwing_move_constructor(const throwing_move_constructor&) = default;

    throwing_move_constructor& operator=(const throwing_move_constructor&) = default;
};
...
struct hash<__gnu_test::rvalstruct> {
    ...
    size_t operator()(const __gnu_test::rvalstruct& __rvs) const { return __rvs.val; }
};
}  // namespace std

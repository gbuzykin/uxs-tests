...
struct NonDefaultConstructible {
    ...
    // For std::iota.
    NonDefaultConstructible& operator++() { return *this; }
#endif
};

// See: 20.1.1 Template argument requirements.
inline bool operator==(const NonDefaultConstructible&, const NonDefaultConstructible&) { return false; }

inline bool operator<(const NonDefaultConstructible&, const NonDefaultConstructible&) { return false; }

// For 23 unordered_* requirements.
struct NonDefaultConstructible_hash {
    std::size_t operator()(NonDefaultConstructible) const { return 1; }
};

// For 26 numeric algorithms requirements, need addable,
// subtractable, multiplicable.
inline NonDefaultConstructible operator+(const NonDefaultConstructible&, const NonDefaultConstructible&) {
...
struct void_function<NonDefaultConstructible> {
    ...
    result_type operator()() const { return result_type(2); }
};
...
struct OverloadedAddress {
    OverloadedAddressAux operator&() const { return OverloadedAddressAux(); }
};

inline bool operator<(const OverloadedAddress&, const OverloadedAddress&) { return false; }

inline bool operator==(const OverloadedAddress&, const OverloadedAddress&) { return false; }

struct OverloadedAddress_hash {
    std::size_t operator()(const OverloadedAddress&) const { return 1; }
};
...
struct NonCopyConstructible {
    ...
    NonCopyConstructible(NonCopyConstructible&& other) : num(other.num) { other.num = 0; }

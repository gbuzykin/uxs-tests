...
template<typename A>
typename A::size_type max_size(const A& a) {
...
class tracker_allocator : public Alloc {
 ...
 public:
    ...
    template<class U>
    struct rebind {
        typedef tracker_allocator<U, typename AllocTraits::template rebind<U>::other> other;
    };
    ...
    tracker_allocator(_Args&&... __args) : Alloc(std::forward<_Args>(__args)...) {}
#else
    tracker_allocator() {}
    ...
    template<class U>
    tracker_allocator(const tracker_allocator<U, typename AllocTraits::template rebind<U>::other>& alloc)
        _GLIBCXX_USE_NOEXCEPT : Alloc(alloc) {}
    ...
    friend inline void swap(tracker_allocator& a, tracker_allocator& b) {
        using std::swap;
...
bool operator==(const tracker_allocator<T1, Alloc1>& lhs, const tracker_allocator<T2, Alloc2>& rhs) throw() {
...
bool check_allocate_max_size() {
    ...
    } catch (std::bad_alloc&) { return true; } catch (...) {
        throw;
    }
    throw;
}
...
class uneq_allocator : private uneq_allocator_base, public Alloc {
    typedef __gnu_cxx::__alloc_traits<Alloc> AllocTraits;

    Alloc& base() { return *this; }
    const Alloc& base() const { return *this; }
    void swap_base(Alloc& b) {
        using std::swap;
        swap(b, this->base());
    }

 public:
    typedef typename check_consistent_alloc_value_type<Tp, Alloc>::value_type value_type;
    typedef typename AllocTraits::size_type size_type;
    typedef typename AllocTraits::pointer pointer;
    ...
    struct rebind {
        typedef uneq_allocator<Tp1, typename AllocTraits::template rebind<Tp1>::other> other;
    };
    ...
    template<typename Tp1>
    uneq_allocator(const uneq_allocator<Tp1, typename AllocTraits::template rebind<Tp1>::other>& b) _GLIBCXX_USE_NOEXCEPT
    ...
    pointer allocate(size_type n, const void* = 0) {
        ...
        try {
            get_map().insert(map_type::value_type(reinterpret_cast<void*>(p), personality));
        } catch (...) {
...
#if __cplusplus >= 201103L
    // Not copy assignable...
    uneq_allocator& operator=(const uneq_allocator&) = delete;

    // ... but still moveable if base allocator is.
    uneq_allocator& operator=(uneq_allocator&&) = default;
#else
 private:
    // Not assignable...
    uneq_allocator& operator=(const uneq_allocator&);
#endif

 private:
    // ... yet swappable!
    friend inline void swap(uneq_allocator& a, uneq_allocator& b) {
        std::swap(a.personality, b.personality);
        a.swap_base(b);
    }

    template<typename Tp1>
    friend inline bool operator==(const uneq_allocator& a,
                                  const uneq_allocator<Tp1, typename AllocTraits::template rebind<Tp1>::other>& b) {
...
class propagating_allocator : public uneq_allocator<Tp, Alloc> {
 ...
 public:
    ...
    struct rebind {
        typedef propagating_allocator<Up, Propagate, typename AllocTraits::template rebind<Up>::other> other;
    };
    ...
    propagating_allocator(
        const propagating_allocator<Up, Propagate, typename AllocTraits::template rebind<Up>::other>& a) noexcept
        : base_alloc(a) {}
    ...
    propagating_allocator& operator=(const propagating_allocator& a) noexcept {
    ...
    template<bool P2>
    propagating_allocator& operator=(const propagating_allocator<Tp, P2, Alloc>& a) noexcept {
    ...
    propagating_allocator(propagating_allocator&& a) noexcept : base_alloc(std::move(a.base())) {}

    // postcondition: LWG2593 a.get_personality() un-changed
    propagating_allocator& operator=(propagating_allocator&& a) noexcept {
...
struct default_init_allocator {
    ...
    template<typename U>
    default_init_allocator(const default_init_allocator<U>& a) : state(a.state) {}
...
bool operator==(const default_init_allocator<T>& t, const default_init_allocator<U>& u) {
...
struct ExplicitConsAlloc : std::allocator<Tp> {
    ...
    template<typename Up>
    explicit ExplicitConsAlloc(const ExplicitConsAlloc<Up>&) {}
...
struct PointerBase : NullablePointer<T*> {
    ...
    PointerBase(const PointerBase<D, U>& p) : NullablePointer<T*>(p.operator->()) {}
 ...
 private:
    friend std::ptrdiff_t operator-(PointerBase l, PointerBase r) { return l.value - r.value; }
...
struct PointerBase_void : NullablePointer<T*> {
    ...
    template<typename D, typename U, typename = decltype(static_cast<T*>(std::declval<U*>()))>
    PointerBase_void(const PointerBase<D, U>& p) : NullablePointer<T*>(p.operator->()) {}
};
...
class memory_resource : public std::pmr::memory_resource {
 public:
    memory_resource() : lists(new allocation_lists) {}
    ...
    void forget_freed_allocations() { lists->forget_allocations(lists->freed); }
...
struct default_resource_mgr {
    explicit default_resource_mgr(std::pmr::memory_resource* r) : prev(std::pmr::set_default_resource(r)) {}

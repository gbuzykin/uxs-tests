 ...
 *  about allocations. Create a uxs::map between the allocated
...
struct annotate_base {
 private:
    ...
    typedef uxs::map<void*, data_type> map_alloc_type;
 ...
 public:
    ...
    static void cleanup() {
        set_label(std::numeric_limits<size_t>::max());
        map_alloc().clear();
        map_construct().clear();
    }
...
struct random_condition : public condition_base {
    ...
    struct group_adjustor : public adjustor_base {
        group_adjustor(size_t size) {
            set_probability(1 - std::pow(double(1 - probability()), double(0.5 / (size + 1))));
        }
    };
...
template<typename _Cond>
inline throw_value_base<_Cond> operator+(const throw_value_base<_Cond>& __a, const throw_value_base<_Cond>& __b) {
    typedef throw_value_base<_Cond> throw_value;
    throw_value::throw_conditionally();
...
template<typename _Tp, typename _Cond>
inline bool operator==(const throw_allocator_base<_Tp, _Cond>&, const throw_allocator_base<_Tp, _Cond>&) {
    return true;
}

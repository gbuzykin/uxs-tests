...
struct setup_base {
    ...
    static size_type generate(size_type __max_size) {
    ...
    struct make_insert_container_base : public make_container_base<_Tp> {
    ...
    template<typename _Tp>
    struct make_insert_container_base<_Tp, false> : public make_container_base<_Tp> {
        using make_container_base<_Tp>::_M_container;
        typedef typename _Tp::value_type value_type;
    ...
    // a size argument.
    template<typename _Tp>
    struct make_container_n<_Tp, true> : public make_container_base<_Tp> {
        make_container_n(const size_type n) : make_container_base<_Tp>(n) {}
    };

    template<typename _Tp>
    struct make_container_n<_Tp, false> : public make_insert_container_base<_Tp> {
        make_container_n(const size_type n) : make_insert_container_base<_Tp>(n) {}
    };
    ...
    template<typename _Tp>
    static bool compare(const _Tp& __control, const _Tp& __test) {
...
struct functor_base : public setup_base {
    ...
    template<typename _Tp, bool = traits<_Tp>::has_erase::value, bool = traits<_Tp>::has_erase_after::value>
    struct erase_point;
    ...
    struct erase_point<_Tp, true, false> : public erase_base<_Tp> {
        ...
        void operator()(_Tp& __container) {
            ...
            } catch (const __gnu_cxx::forced_error&) { throw; }
        }
    };
    ...
    template<typename _Tp, bool = traits<_Tp>::has_insert::value, bool = traits<_Tp>::has_insert_after::value>
    struct insert_point;
    ...
    template<typename _Tp, bool = traits<_Tp>::has_emplace::value &&
                                  (traits<_Tp>::is_associative::value || traits<_Tp>::is_unordered::value)>
    struct emplace;
    ...
    template<typename _Tp, bool = traits<_Tp>::has_emplace::value,
             bool = traits<_Tp>::is_associative::value || traits<_Tp>::is_unordered::value,
    ...
    // Specialization, empty.
    template<typename _Tp, bool is_associative_or_unordered, bool has_insert_after>
    struct emplace_point<_Tp, false, is_associative_or_unordered, has_insert_after> {
        void operator()(_Tp&) {}

        void operator()(_Tp&, _Tp&) {}
    };

    template<typename _Tp, bool = traits<_Tp>::is_associative::value || traits<_Tp>::is_unordered::value>
    struct clear {
        void operator()(_Tp& __container) {
            try {
                __container.clear();
...
struct basic_safety : public test_base<_Tp> {
    ...
    size_t run_steps_to_limit(size_t __step, container_type& __cont, const _Funct& __f) {
        bool exit(false);
...
struct propagation_consistent : public test_base<_Tp> {
    ...
    template<typename _Funct>
    void run_steps_to_limit(container_type& container_control, const _Funct& __f) {
        size_t i(1);
        bool exit(false);

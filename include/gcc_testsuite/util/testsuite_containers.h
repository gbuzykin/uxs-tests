...
template<typename _Iterator, bool _Mutable, typename = typename std::iterator_traits<_Iterator>::iterator_category>
...
template<typename _Iterator>
struct iterator_concept_checks<_Iterator, false, std::bidirectional_iterator_tag> {
    iterator_concept_checks() {
        using namespace __gnu_cxx;
...
template<typename _Iterator>
struct iterator_concept_checks<_Iterator, true, std::bidirectional_iterator_tag> {
    iterator_concept_checks() {
        using namespace __gnu_cxx;
...
template<typename _Iterator>
struct iterator_concept_checks<_Iterator, false, std::random_access_iterator_tag> {
    iterator_concept_checks() {
        using namespace __gnu_cxx;
...
template<typename _Iterator>
struct iterator_concept_checks<_Iterator, true, std::random_access_iterator_tag> {
    iterator_concept_checks() {
        using namespace __gnu_cxx;
...
struct forward_members {
    forward_members(_Tp& container) {
        ...
        iterator_concept_checks<typename _Tp::iterator,
                                !(traits_type::is_associative::value || traits_type::is_unordered::value)>
            cc;
...
template<typename _Tp>
struct category_members<_Tp, std::random_access_iterator_tag> : forward_members<_Tp> {
    category_members(_Tp& container) : forward_members<_Tp>(container) {
        // Make sure that even if rel_ops is injected there is no ambiguity
        // when comparing iterators.
        using namespace std::rel_ops;

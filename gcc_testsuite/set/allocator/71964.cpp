...
struct mv_allocator {
    ...
    mv_allocator() : moved_to(false), moved_from(false) {}

    template<typename U>
    mv_allocator(const mv_allocator<U>& a) : moved_to(a.moved_to), moved_from(a.moved_from) {}
...
bool operator==(const mv_allocator<T>&, const mv_allocator<U>&) {
    return true;
}
...
int test01() {
    uxs::set<int, std::less<int>, mv_allocator<int>> s;
    auto t = std::move(s);

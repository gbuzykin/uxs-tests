...
struct OutputContainer : public BoundsContainer<T> {
    ...
    OutputContainer(T* _first, T* _last)
        : BoundsContainer<T>(_first, _last), incrementedto(_first), writtento(new bool[this->size()]()) {}
...
class WritableObject {
 ...
 public:
    ...
    WritableObject(T* ptr_in, OutputContainer<T>* SharedInfo_in) : ptr(ptr_in), SharedInfo(SharedInfo_in) {}

#if __cplusplus >= 201103L
    template<class U>
    typename std::enable_if<std::is_assignable<T&, U>::value>::type operator=(U&& new_val) const {
...
struct output_iterator_wrapper {
 ...
 public:
    ...
    output_iterator_wrapper(T* _ptr, ContainerType* SharedInfo_in) : ptr(_ptr), SharedInfo(SharedInfo_in) {
    ...
    output_iterator_wrapper& operator=(const output_iterator_wrapper&) = default;
#endif
...
class input_iterator_wrapper {
 ...
 public:
    ...
    bool operator!=(const input_iterator_wrapper& in) const { return !(*this == in); }
...
struct forward_iterator_wrapper : public input_iterator_wrapper<T> {
    ...
    forward_iterator_wrapper(T* _ptr, ContainerType* SharedInfo_in) : input_iterator_wrapper<T>(_ptr, SharedInfo_in) {}
    ...
    forward_iterator_wrapper& operator=(const forward_iterator_wrapper&) = default;
#endif
    ...
    bool operator!=(const forward_iterator_wrapper& it) const noexcept { return !(*this == it); }
#endif
};
...
#if __cplusplus >= 201103L
    bidirectional_iterator_wrapper(const bidirectional_iterator_wrapper&) = default;

    bidirectional_iterator_wrapper& operator=(const bidirectional_iterator_wrapper&) = default;
#endif
...
struct random_access_iterator_wrapper : public bidirectional_iterator_wrapper<T> {
    ...
    random_access_iterator_wrapper& operator-=(std::ptrdiff_t n) { return *this += -n; }
    ...
    bool operator>(const random_access_iterator_wrapper<T>& in) const { return in < *this; }

    bool operator>=(const random_access_iterator_wrapper<T>& in) const { return !(*this < in); }

    bool operator<=(const random_access_iterator_wrapper<T>& in) const { return !(*this > in); }
};

template<typename T>
random_access_iterator_wrapper<T> operator+(random_access_iterator_wrapper<T> it, std::ptrdiff_t n) {
    return it += n;
}

template<typename T>
random_access_iterator_wrapper<T> operator+(std::ptrdiff_t n, random_access_iterator_wrapper<T> it) {
    return it += n;
}
...
template<typename T>
using bidirectional_container = test_container<T, bidirectional_iterator_wrapper>;
...
struct contiguous_iterator_wrapper : random_access_iterator_wrapper<T> {
    ...
    contiguous_iterator_wrapper& operator+=(std::ptrdiff_t n) {
        random_access_iterator_wrapper<T>::operator+=(n);
        return *this;
    }

    friend contiguous_iterator_wrapper operator+(contiguous_iterator_wrapper iter, std::ptrdiff_t n) {
        return iter += n;
    }

    friend contiguous_iterator_wrapper operator+(std::ptrdiff_t n, contiguous_iterator_wrapper iter) {
        return iter += n;
    }

    contiguous_iterator_wrapper& operator-=(std::ptrdiff_t n) { return *this += -n; }
...
struct input_iterator_wrapper_nocopy : input_iterator_wrapper<T> {
    ...
    input_iterator_wrapper_nocopy() : input_iterator_wrapper<T>(nullptr, nullptr) {}

    input_iterator_wrapper_nocopy(const input_iterator_wrapper_nocopy&) = delete;
    input_iterator_wrapper_nocopy& operator=(const input_iterator_wrapper_nocopy&) = delete;

    input_iterator_wrapper_nocopy(input_iterator_wrapper_nocopy&&) = default;
    input_iterator_wrapper_nocopy& operator=(input_iterator_wrapper_nocopy&&) = default;
...
struct test_sized_range : test_range<T, Iter> {
    ...
    std::size_t size() const noexcept { return this->bounds.size(); }
};

template<typename T>
using test_contiguous_sized_range = test_sized_range<T, contiguous_iterator_wrapper>;
template<typename T>
using test_random_access_sized_range = test_sized_range<T, random_access_iterator_wrapper>;
template<typename T>
using test_bidirectional_sized_range = test_sized_range<T, bidirectional_iterator_wrapper>;
template<typename T>
...
struct test_sized_range_sized_sent : test_sized_range<T, Iter> {
    ...
    struct sentinel {
        ...
        friend std::iter_difference_t<I> operator-(const sentinel& s, const I& i) noexcept { return s.end - i.ptr; }

        friend std::iter_difference_t<I> operator-(const I& i, const sentinel& s) noexcept { return i.ptr - s.end; }
    };

...
int test01() {
    ...
    Map::iterator p = m.insert(m.begin(), Pair(rvalstruct(1), rvalstruct(3)));
    VERIFY(m.size() == 1);
...
int test02() {
    ...
    Map::iterator p1 = m.insert(m.begin(), Pair(rvalstruct(2), rvalstruct(3)));
    Map::iterator p2 = m.insert(p1, Pair(rvalstruct(2), rvalstruct(7)));
    VERIFY(m.size() == 1);
    VERIFY(p1 == p2);

...
size_t uxs_to_chars_int64(char* first, char* /*last*/, int64_t val) { return uxs::to_chars(first, val) - first; }
size_t uxs_from_chars_int64(std::string_view s, int64_t& val) {
    return uxs::from_chars(s.data(), s.data() + s.size(), val).ptr - s.data();
}
size_t uxs_to_chars_double(char* first, char* /*last*/, double val) { return uxs::to_chars(first, val) - first; }
size_t uxs_to_chars_double_prec(char* first, char* /*last*/, double val, int prec) {
    return uxs::to_chars(first, val, uxs::fmt_opts{uxs::fmt_flags::none, prec}) - first;
}
...
size_t std_to_chars_int64(char* first, char* last, int64_t val) { return std::to_chars(first, last, val).ptr - first; }
size_t std_from_chars_int64(std::string_view s, int64_t& val) {
    return std::from_chars(s.data(), s.data() + s.size(), val).ptr - s.data();
}
#    if defined(has_to_chars_implementation_for_floats)
size_t std_to_chars_double(char* first, char* last, double val) { return std::to_chars(first, last, val).ptr - first; }
size_t std_to_chars_double_prec(char* first, char* last, double val, int prec) {
    return std::to_chars(first, last, val, std::chars_format::general, prec).ptr - first;
}
#    endif
...
size_t fmt_format_to_int64(char* first, char* /*last*/, int64_t val) {
    return fmt::format_to(first, FMT_COMPILE("{}"), val) - first;
}
...
size_t fmt_format_to_double_prec(char* first, char* /*last*/, double val, int prec) {
    return fmt::format_to(first, FMT_COMPILE("{:.{}}"), val, prec) - first;
}

#if defined(has_cpp_lib_format)
size_t std_format_to_int64(char* first, char* /*last*/, int64_t val) {
    return std::format_to(first, "{}", val) - first;
}
...
size_t std_format_to_double_prec(char* first, char* /*last*/, double val, int prec) {
    return std::format_to(first, "{:.{}}", val, prec) - first;
}
#endif

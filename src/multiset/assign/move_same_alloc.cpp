...
int test_move_empty_to_empty() {
    ...
    {
        uxs::multiset<Ty, std::less<Ty>, test_allocator<Ty>> ms(al), ms_from(al);
        ms = std::move(ms_from);
        CHECK_EMPTY(ms);
...
int test_move_not_empty_to_empty() {
...

    {
        ...
        uxs::multiset<Ty, std::less<Ty>, test_allocator<Ty>> ms(al), ms_from(init, al);
        ms = std::move(ms_from);
...
int test_move_not_empty_to_not_empty() {
...

    {
        ...
        uxs::multiset<Ty, std::less<Ty>, test_allocator<Ty>> ms(init, al), ms_from(init2, al);
        ms = std::move(ms_from);
...
int test_move_empty_to_not_empty() {
    ...
    {
        std::initializer_list<Ty> init = {1, 2, 3, 4, 5};
        uxs::multiset<Ty, std::less<Ty>, test_allocator<Ty>> ms(init, al), ms_from(al);
        ms = std::move(ms_from);
        CHECK_EMPTY(ms);
        VERIFY(ms.get_allocator() == al);
        CHECK_EMPTY(ms_from);

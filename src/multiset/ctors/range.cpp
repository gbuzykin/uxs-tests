...
int test_range_empty() {
    ...
    {
        Src init;
        uxs::multiset<T, std::less<T>, test_allocator<T>> ms(init.begin(), init.end(), al);
        CHECK_EMPTY(ms);
...
ADD_TEST_CASE("", "multiset", test_range_empty);
ADD_TEST_CASE("", "multiset", test_range_not_empty);

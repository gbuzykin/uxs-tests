...
int test_move_empty_to_empty() {
    ...
    {
        util::vector<Ty, unfriendly_test_allocator<Ty>> v(al), v_from(al2);
        v = std::move(v_from);
        CHECK_EMPTY(v);
        VERIFY(v.capacity() == 0);
...
int test_move_not_empty_to_empty() {
...

    {
        ...
        util::vector<Ty, unfriendly_test_allocator<Ty>> v(al), v_from(tst, al2);
        v = std::move(v_from);
...
int test_move_more_no_realloc() {
...

    {
        ...
        util::vector<Ty, unfriendly_test_allocator<Ty>> v(tst_prev, al), v_from(tst, al2);
        v.reserve(10);
...
int test_move_empty_to_not_empty() {
    ...
    {
        std::initializer_list<Ty> tst_prev = {1, 2, 3, 4, 5};
        util::vector<Ty, unfriendly_test_allocator<Ty>> v(tst_prev, al), v_from(al2);
        v = std::move(v_from);
        CHECK_EMPTY(v);
        VERIFY(v.get_allocator() == al);
        VERIFY(v_from.capacity() == 0);

...
int test_copy_empty_to_empty() {
    ...
    {
        util::vector<Ty, unfriendly_test_allocator<Ty>> v(al), v_from(al);
        v = v_from;
        CHECK_EMPTY(v);
        VERIFY(v.capacity() == 0);
...
int test_copy_not_empty_to_empty() {
...

    {
        ...
        util::vector<Ty, unfriendly_test_allocator<Ty>> v(al), v_from(tst, al);
        v = v_from;
        CHECK(v, tst.size(), tst.begin());
        VERIFY(v.capacity() >= v.size());
        VERIFY(v.get_allocator() == al);
        VERIFY(Ty::instance_count == 15);
        VERIFY(Ty::not_empty_count == 15);
...
int test_copy_more_no_realloc() {
...

    {
        ...
        util::vector<Ty, unfriendly_test_allocator<Ty>> v(tst_prev, al), v_from(tst, al);
        v.reserve(10);
        ...
        VERIFY(v.capacity() >= v.size());
        VERIFY(v.get_allocator() == al);
        VERIFY(Ty::instance_count == 26);
        VERIFY(Ty::not_empty_count == 26);
        VERIFY(al.get_alloc_detected() == 0);
    }
...
int test_copy_less() {
...

    {
        ...
        VERIFY(v.capacity() >= v.size());
        VERIFY(v.get_allocator() == al);
        VERIFY(Ty::instance_count == 22);
        VERIFY(Ty::not_empty_count == 22);
        VERIFY(al.get_alloc_detected() == v.capacity() + v_from.capacity());
    }
...
int test_copy_same_amount() {
...

    {
        ...
        VERIFY(v.capacity() >= v.size());
        VERIFY(v.get_allocator() == al);
        VERIFY(Ty::instance_count == 20);
        VERIFY(Ty::not_empty_count == 20);
        VERIFY(al.get_alloc_detected() == v.capacity() + v_from.capacity());
    }
...
int test_copy_empty_to_not_empty() {
    ...
    {
        std::initializer_list<Ty> tst_prev = {1, 2, 3, 4, 5};
        util::vector<Ty, unfriendly_test_allocator<Ty>> v(tst_prev, al), v_from(al);
        v = v_from;
        CHECK_EMPTY(v);
        VERIFY(v.get_allocator() == al);

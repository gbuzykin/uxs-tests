...
int test_copy_not_empty_to_empty() {
...

    {
        ...
        uxs::list<Ty, unfriendly_test_allocator<Ty>> l(al), l_from(tst, al);
        l = l_from;
...
int test_copy_more() {
...

    {
        ...
        uxs::list<Ty, unfriendly_test_allocator<Ty>> l(tst_prev, al), l_from(tst, al);
        l = l_from;
        CHECK(l, tst.size(), tst.begin());
        VERIFY(l.get_allocator() == al);
...
int test_copy_empty_to_not_empty() {
    ...
    {
        std::initializer_list<Ty> tst_prev = {1, 2, 3, 4, 5};
        uxs::list<Ty, unfriendly_test_allocator<Ty>> l(tst_prev, al), l_from(al);
        l = l_from;
        CHECK_EMPTY(l);
        VERIFY(l.get_allocator() == al);
